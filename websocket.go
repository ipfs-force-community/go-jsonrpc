	...
	go func() {
		for {
			...
			case <-time.After(c.pingInterval):
				log.Debugf("send ping request %s", c.conn.RemoteAddr())
				c.writeLk.Lock()
				if err := c.conn.WriteMessage(websocket.PingMessage, []byte{}); err != nil {
					log.Errorf("sending ping message: %+v", err)
				}
				log.Debugf("finish send ping request %s", c.conn.RemoteAddr())
				c.writeLk.Unlock()
			case <-stop:
				return
			}
...
func (c *wsConn) handleWsConn(ctx context.Context) {
	...
	for {
		...
		case <-c.pongs:
			log.Debugf("receive pong from %s", c.conn.RemoteAddr())
			if c.timeout > 0 {
				if err := c.conn.SetReadDeadline(time.Now().Add(c.timeout)); err != nil {
					log.Error("setting read deadline", err)
				}
